...
function embaralharQuestoes(arrayPerguntas) {
    return arrayPerguntas.sort(() => Math.random() - 0.5);
}

function selecionarQuestoesAleatorias(arrayPerguntas, quantidade) {
    const questoesEmbaralhadas = embaralharQuestoes(arrayPerguntas);
    return questoesEmbaralhadas.slice(0, quantidade);
}

function imprimirPerguntas(arrayPerguntas) {

    arrayPerguntas.forEach((questao, index) => {
        console.log(`\n${index + 1}: ${questao.pergunta}`);

        const respostaUsuario = entradaDados.question('Digite a resposta (ano): ').toString();
        if (respostaUsuario.trim().toLowerCase() === questao.resposta.toLowerCase()) {
...
const perguntasAleatorias = selecionarQuestoesAleatorias(questoes, numeroPerguntas);

console.log("----- QUIZ DE FATOS HISTÓRICOS -----");
console.log("\nSeja Bem-vindo jogador(a)!")
const nome = entradaDados.question('\nDigite seu nome: ');

imprimirPerguntas(perguntasAleatorias);
